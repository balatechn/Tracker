              ...
              {filtered.flatMap((emp, i) => {
                const rows = [
                  <tr key={emp.id} className={`border-b border-gray-100 hover:bg-blue-50 ${i % 2 === 1 ? 'bg-gray-50/50' : ''}`}>
                    ...
                    <td className="px-3 py-2 w-[220px] min-w-[220px]">
                      {emp.msLicensed === true
                        ? <span className="px-2 py-0.5 rounded text-xs font-semibold bg-green-50 text-green-700 block leading-tight">{emp.msLicenseName ?? '—'}</span>
